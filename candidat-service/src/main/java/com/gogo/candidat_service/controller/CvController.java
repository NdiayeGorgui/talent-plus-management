...
public class CvController {
    ...
    public ResponseEntity<CvDTO> replaceCv(
            ...
            @RequestParam("file") MultipartFile file,
            @RequestParam(value = "titre", required = false) String titre) throws IOException {
        return ResponseEntity.ok(cvService.replaceCv(cvId, file, titre));
    }

    // 🔹 Supprimer un CV
    @DeleteMapping("/{cvId}")
    public ResponseEntity<Void> deleteCv(@PathVariable("cvId") Long cvId) {
    ...
    public ResponseEntity<List<CvDTO>> getCvsByCandidat(@PathVariable("candidatId") Long candidatId) {
